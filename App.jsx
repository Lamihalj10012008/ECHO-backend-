...
export default function App() {
  // Input fields hook tracking
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');
  const [role, setRole] = useState('Student');
  
  // Track system view configuration (Login vs Registration)
  const [isRegisterMode, setIsRegisterMode] = useState(false);

  // Track active authenticated user details
  const [user, setUser] = useState(null);

  // Status message tracking 
  const [alert, setAlert] = useState({ message: '', type: '' });

  // Lifecycle session recovery & Bootstrap injection
  useEffect(() => {
  ...
  // Universal dynamic submission processor
  const handleAuthSubmit = async (e) => {
    e.preventDefault();
    setAlert({ message: '', type: '' });

    // Determine target API endpoint dynamically
    ...
    try {
      ...
      if (response.ok) {
        if (isRegisterMode) {
          // If registration is successful, reset states and prompt login
          setAlert({ message: data.message, type: 'success' });
          setIsRegisterMode(false);
          setPassword('');
        } else {
          // If login is successful, persist session context
          const loggedInUser = { username, role, message: data.message };
          localStorage.setItem('echo_user', JSON.stringify(loggedInUser));
          setUser(loggedInUser);
        }
      } else {
        // Intercepts and logs validation layout errors directly from backend
        setAlert({
  ...
  // Styled design tokens layout rules
  const styles = {
    bodyWrapper: {
  ...
  // --- VIEW B: GATEWAY VIEW (LOGIN / REGISTER) ---
  return (
    <div style={styles.bodyWrapper}>
